...
### Externally defined flags types

If you're generated flags types for an external API, you can define an extra unnamed flag
as a mask of all bits the external source may set. Usually this would be all bits (`!0`):
...
may start setting additional bits at any time.
...
See the docs for the [`Flags`] trait for more details on operators.
...
Flags types generated by `bitflags` don't guarantee that only bits in flags you define can ever be set.
Bits outside of flags you define are called _unknown bits_. Most operators will treat these unknown bits
the same as known ones, with the exception of `!`, which will unset them. This behavior may be
tightened in future major versions of `bitflags`.

If you're using `bitflags` for flags types defined externally, such as when binding to C APIs, you
can use an unnamed flag to guarantee all bits are known bits. See the
[externally defined flags types](#externally-defined-flags-types) section for more details.
*/
